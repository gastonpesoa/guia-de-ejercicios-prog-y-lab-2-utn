...
namespace PrimerParcial
{
    public partial class frmPrincipal : Form
    {
        ...
        private void cargarComensales()
        {
            ...
            if (cmbMesas.SelectedIndex >= 0)
            {
                lstComensales.Items.Add(mesas[cmbMesas.SelectedIndex].ToString());
                foreach (Comensal comensal in mesas[cmbMesas.SelectedIndex].Comensales)
                {
                    if (comensal is Mayor)
                    {
                        lstComensales.Items.Add(((Mayor)comensal).ToString());
                    }
                    else
                    {
                        lstComensales.Items.Add(comensal.Mostrar());
                    }
